    ...
    tbody.innerHTML = plans.map(plan => {
        ...
        return `
            <tr>
                ...
                <td>
                    <span class="status-indicator-box in-progress">In Progress</span>
                </td>
                <td>
...
function populateRecipeDropdown(recipes) {
    ...
    if (!recipes || recipes.length === 0) {
        select.innerHTML = `
            <option value="1">Coffee Jelly Standard</option>
            <option value="2">Buko Pandan Jelly</option>
        `;
        return;
    }
...
function populateSupervisorDropdown(staff) {
    ...
    if (!staff || staff.length === 0) {
        select.innerHTML = '<option value="1">Admin / Staff</option>';
        return;
    }
...
async function handleCreatePlan(e) {
    ...
    if (!recipe_id || !total_cups_produced) {
    ...
    try {
        const response = await fetch('/api/admin/create-plan', {
            ...
            body: JSON.stringify({
                ...
                cooked_by: parseInt(cooked_by, 10) || 1
            })
        });
